const MessageModel = require("./../../Model/Vendor/Messages");
...
exports.handleSendMessages = async (req, resp, next) => {
  ...
  try {
    const newMessage = new MessageModel({ vendorId, title, message, date });
...
exports.handleGetMessages = async (req, resp, next) => {
  ...
  try {
    const res = await MessageModel.find({ vendorId });
